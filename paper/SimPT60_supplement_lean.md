...
**表注。** 表内是夏季基准默认值，非所有线路的最终值；可匹配公开回路的线路使用逐字段来源覆盖。电缆修正系数 r ×0.65、x ×0.35、c ×20、Imax ×0.90。按季节再施加 Table 6 中的定额倍率。来源 model_config.json。
...
**变压器表注。** 默认容量与阻抗为工程代理；公开容量优先，分电压组合总容量不足时以 REN 汇总向上校准并保留系数。档位参数见 Table 6；Estoi 等逐设备覆盖值不能被此默认表覆盖。

...
公开可获取的电网数据通常分散于网络结构、设备属性和运行记录中，难以直接用于连续潮流分析和方法比较。本文提出 SimPT60，一个基于多源公开数据构建的葡萄牙高压电网研究数据集，覆盖葡萄牙大陆 60–400 kV 网络及葡西互联边界。该数据集将静态网络、运行时序和交流潮流结果组织在统一的数据框架中，并提供连续的 15 分钟运行案例、来源追踪信息以及用于评估模型一致性的数据产品。由此，SimPT60 可用于时序潮流分析、情景比较、相对风险筛查和电网算法测试。作为应用示例，本文进一步通过**未参与节点功率构建的 E-REDES 公开数据**、参数敏感性分析和 N−1 故障筛查展示其研究可用性，同时明确其并非经过设备级验证的运营商网络模型。
...
电网运行、规划和风险评估研究通常依赖潮流仿真，而此类研究需要描述网络拓扑、设备属性和运行条件的电网数据 ([Thurner et al., 2018](#ref-Thurner2018))。随着研究从单一运行状态扩展到时序分析，静态网络还需要与连续的负荷、发电和其他运行序列相结合 ([Hörsch et al., 2018](#ref-Horsch2018); [Meinecke et al., 2020](#ref-Meinecke2020))。

现有公开资源已经提供了多种电网模型和 benchmark datasets。PyPSA-Eur 提供面向欧洲能源系统优化和运行研究的输电网模型 ([Hörsch et al., 2018](#ref-Horsch2018))；近期基于开放数据的欧洲高压电网重建工作扩展了真实地理网络的公开表示 ([Xiong et al., 2025](#ref-Xiong2025))；SimBench 则提供跨多个电压等级的代表性网络以及全年负荷和发电时间序列 ([Meinecke et al., 2020](#ref-Meinecke2020))。这些资源分别服务于系统优化、网络重建和方法比较等研究目的，并形成了公开电网研究的重要数据基础。

## 1.1 Motivation

葡萄牙能源和地质总局（Direção-Geral de Energia e Geologia, DGEG）的能源与气候规划将可再生能源、储能、电气化和跨境互联列为能源转型的重要路径 ([DGEG, 2024](#ref-DGEGPNEC2030Revision2024))。随着这些变化持续推进，发电与负荷的规模和空间分布也会发生变化，从而影响线路潮流、节点电压和系统运行风险。因此，研究葡萄牙电网的时序运行特征，需要能够同时描述网络结构和连续运行条件的数据基础。

然而，相关公开记录目前分散在 E-REDES、葡萄牙国家能源网络公司（Redes Energéticas Nacionais, REN）、DGEG、能源服务监管局（Entidade Reguladora dos Serviços Energéticos, ERSE）以及开放地理数据平台 ([E-REDES, n.d.-f](#ref-EREDESRND2026); [REN, n.d.](#ref-RENDataHub2026); [DGEG, n.d.](#ref-DGEGGeo2026); [REN, 2024](#ref-ERSEPDIRT2024); [OpenStreetMap contributors, n.d.](#ref-OSM2026))。不同来源的空间覆盖、时间分辨率、对象标识和字段定义并不一致。将这些数据用于统一的潮流分析，还需要完成网络拓扑重建、电气参数补充、资产—母线映射、时间对齐以及聚合功率向节点的分配。

现有公开资源也具有不同的建模目标和覆盖范围。例如，欧洲级模型主要服务于跨区域系统研究 ([Hörsch et al., 2018](#ref-Horsch2018))，开放高压网络重建主要覆盖 220 kV 及以上交流网络 ([Xiong et al., 2025](#ref-Xiong2025))，而代表性 benchmark datasets 并不描述葡萄牙实际地理网络 ([Meinecke et al., 2020](#ref-Meinecke2020))。因此，目前仍缺少一个能够同时连接葡萄牙大陆 60–400 kV 网络、15 分钟运行时序、可计算潮流案例和明确来源追踪的公开研究数据基础。

因此，本研究的目标是建立一个可追溯、可计算并具有连续时间案例的葡萄牙高压电网数据集，为时序潮流分析、电网风险筛查、情景比较和方法测试提供统一的数据基础。
...
SimPT60 的核心贡献是把葡萄牙大陆 60–400 kV 的可追溯公开数据网络，与 31,492 个连续、可复算的 15 分钟交流潮流状态连接在同一版本化数据产品中。原始记录、派生对象及验证结果分层保存，支持回溯关键字段的证据；其适用边界集中见第 4.7 节。数据集的覆盖范围、核心数据产品与解释边界汇总于 Table 1。


**Table 1——SimPT60 的范围与数据产品。**
...
Figure 1 示意连接判定，Table 3 汇总阈值与未匹配处理。

![Figure 1](figures_final/fig01_network_reconstruction.png)

**Figure 1——静态网络重建中的连接规则。** (a) 同电压端点聚类与设施匹配；(b) 区分几何交叉和显式共享节点；(c) 保留串联分段与并行回路身份；(d) 根据显式资产、同址设施和 RARI 边界证据连接不同电压层。图为抽象电气示意，不表示真实设备坐标；距离阈值及回退规则见 Table 3。
...
静态来源日期与运行案例日期分别版本化。只有存在明确投运日期的资产才按时点切换；普通线路、变压器和互联设施在缺少同期状态记录时保留冻结静态状态。项目文件对接入和参数的支撑按适用设备及字段记录，不能由规划资料推断实际开关状态；导线电阻的截面缩放和向其他导线族转用仍属于工程假设。证据页码、设备 ID 和逐字段判断保存在 `citation_evidence_ledger.csv`，版本处理汇总于 Table 4。

**Table 4——来源版本、有效日期与案例处理。**
...
资产映射、时间对齐和守恒处理的可执行规则汇总于 Table 5。
...
交流潮流采用 pandapower Newton–Raphson 算法和 DC 初始化 ([Thurner et al., 2018](#ref-Thurner2018))。主流程执行无功限值，并在边界重构后进行一次重新求解和有限分接搜索。季节定额、PV/PQ 规则、补偿、电抗器、边界权重和数值设置汇总于 Table 6；这些统一设置是可计算代理，不代表运营商控制策略。

### 2.4.2 Monthly Execution

案例按自然月并行计算，工作进程只读输入并独立求解，父进程在单一事务中写入摘要、状态数组和审计包。重新运行跳过完整案例并重算失败项；只有预期数、完成数、数组数和审计包数一致的月份才标记为完整。主案例设置见 Table 6。
...
静态网络、时间序列、验证和应用数据的核心规模见 Table 7。
...
**Figure 4——结构覆盖与逐参数来源支撑。** (a) 各电压等级的模型 route-km / 公开背景长度比值，虚线为 1；灰色圆点对应非独立 OSM 背景，蓝色方点对应 REN 背景。route-km 与回路长度口径不同，比值不是准确率。(b) 按字段区分项目电阻转用（橙色，工程假设）与有来源定额（绿色），单元格给出对应比例；灰色表示全为工程代理。R 的 35.8% 不是直接参数观测，不能与电流定额的证据等级合并；X、C 均为代理。130 kV 仅 7 条线路，长度比仅供参考。线路级部分支撑计数仍见 Table 8。


网络覆盖、参数证据和计算完整性的汇总指标见 Table 8。

**Table 8——网络结构、参数证据与计算完整性。**
...
**Figure 6——聚合输入的外部时间验证。** (a–b) 全国消费与风电的成对日平均；(c–d) 对应 31,388 与 31,484 个 15 分钟配对，六边形密度共用对数计数色标，虚线为一比一线，r 为 Pearson 系数；(e) 同一消费配对集合按 Lisbon 当地时刻聚合后、各自除以日内均值的曲线。各比较仅排除所需字段缺失的区间，不删除异常月份、不平滑或插值。消费不含抽水和电池充电，与案例总电气负荷区分；风电为全国生产与配网注入的跨范围印证。该图验证聚合输入，不验证节点分配或支路潮流；重采样置信区间（confidence interval, CI）见 Table 9。
...
跨来源验证的样本量、区间估计和证据层级汇总于 Table 9。
...
整站留出实验将 394 个可匹配站按固定规则分为五折，并在 22 个时点比较全局容量比例、地理邻站和重建网络邻站三种预测，共形成 8,541 个观测对。Table 10 显示网络邻站法的 MAE 比全局容量比例低约 9.9%，但没有优于地理邻站法，且置信区间重叠。因此，重建连接包含有用邻近信息，却不能单独恢复精确节点负荷；完整 fold、距离权重和 bootstrap 协议见补充材料 [Note S3](SimPT60_supplement_lean.md#supplementary-note-s3)。

**Table 10——整站留出的空间分配结果。**
...
六个时点共形成 9,894 个案例，9,888 个主潮流收敛；6,048 个通过绝对筛查，9,064 个没有相对 N−0 新增违规且无实质孤岛。后者是辅助诊断，不能识别所有已有越限恶化，也不能替代绝对判据。Figure 10 和 Table 11 给出结果；完整成员和逐案例输出保存在仓库。

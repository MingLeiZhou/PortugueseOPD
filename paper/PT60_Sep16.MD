...
导线电阻依据 2019-02-19 的 PE Tocha II–Tocha 60 kV 项目说明（工程号 2800-19C007374），PDF 第 3、5 页，印刷第 2、4 页。文档给出的钢芯铝绞线（aluminium conductor steel-reinforced, ACSR）总截面为 326.1 mm²、铝截面 264.4 mm²、钢截面 61.7 mm²，20°C 电阻为 0.1093 Ω/km。现有代码以名义截面 \(A\) 按 \(R=0.1093\times325/(A n)\) 转用，\(n\) 由“2x”前缀取 2，否则取 1，再按串联段长度加权。该截面缩放、前缀解释及向其他导线族的转用均为工程假设；没有逐线验证交流效应、运行温度、材质或束导线含义。核心库中 1,517 行的历史 `CONDUCTOR_DERIVED` 标签因此在发布的 `resistance_evidence_overlay.csv` 中统一降级为“项目参数转用工程假设”，另 3,426 行继续标为工程代理。原始数值和字节哈希不变，不能再将前者计作直接电阻观测。
...
对原 264 个扰动结果重新计算每行线路进入 Top-20 的频率。每时点 12 种配置（基线及 11 种扰动）分别排名；并列值按稳定线路 ID 排序。定义 \(f(i,t)=\text{入选次数}/\text{该时点成功配置数}\)，\(f\ge 0.8\) 为“该时点持续热点”，\(0<f<0.8\) 为“依赖假设的热点”；未入选为 0。跨时点总频率另报，避免把季节变化误当参数不稳定。
...
![Figure 6](figures_final/fig06_hotspot_persistence.png)
...
![Figure 7](figures_final/fig07_temporal_validation.png)
...
![Figure 8](figures_final/fig08_spatial_validation.png)
...
![Figure 9](figures_final/fig09_generation_balance_scope.png)
...
![Figure 10](figures_final/fig10_proxy_provenance.png)
...
![Figure 11](figures_final/fig11_grid_stress.png)
...
![Figure 12](figures_final/fig12_annual_nminus1.png)

...
The selected configuration remained node set B, a 100~m facility buffer, a 0.5~m endpoint snap threshold, and voltage-plus-status-aware merging. Both builds retained 358 branches, but the equality of totals did not imply equality of records: 357 of 358 retained source-line groups (99.72\%) preserved their facility endpoint pair, the ANTAS--PARANHOS branch from the legacy build was removed, and the SÃO JOÃO DE PONTE--SENHORA DO PORTO branch entered the final release. The corresponding release-local branch identifiers and source-line membership are recorded in \path{validation/projection\_release\_transition.csv}; the summary is in \path{validation/projection\_release\_transition\_summary.json}. \dataset{} consequently contains 1,341 circuit candidates, comprising 358 retained and 983 downgraded records. This is reported as a measured projection transition, not as projection invariance.

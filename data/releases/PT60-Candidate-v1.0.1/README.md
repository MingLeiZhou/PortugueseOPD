...
Code DOI remains pending; the public source repository is recorded in the associated article. See `manifest.json`, `checksums.sha256`, `schema/`, `inventory/headline_counts.json` and `excluded_artifacts.json` for archive contents, schemas, checksums and exclusions.

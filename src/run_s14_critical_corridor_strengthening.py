"""S14 diagnostic scenario: strengthen critical corridors ATPL_00075 and ATPL_00147."""
...
def load_net_checked():
    ...
    if readiness not in ALLOWED_READINESS:
        raise RuntimeError(f"Fail-closed: readiness status {readiness!r} is not allowed for S14 diagnostic scenario.")
    return net, opts
...
def set_slack(net) -> None:
    ...
    pp.create_ext_grid(net, bus=SLACK_BUS, vm_pu=1.0, va_degree=0.0, name=f"S14_ext_grid_{SLACK_BUS}")
    ...
    opts.update(
        {
            ...
            "opf_ready": False,
            "provenance_policy": "diagnostic_critical_corridor_strengthening",
...
def apply_variant(net, variant: dict[str, Any]) -> dict[str, Any]:
    ...
    modified = line[mask].copy().reset_index(names="line_index")
    if variant["variant_id"] == "baseline":
        return {"modified_line_count": 0}
    if variant["variant_id"] == "max_i_2x":
        net.line.loc[mask, "max_i_ka"] = pd.to_numeric(net.line.loc[mask, "max_i_ka"], errors="coerce") * 2.0
    elif variant["variant_id"] == "max_i_5x":
        net.line.loc[mask, "max_i_ka"] = pd.to_numeric(net.line.loc[mask, "max_i_ka"], errors="coerce") * 5.0
    elif variant["variant_id"] == "x_0p7":
        net.line.loc[mask, "x_ohm_per_km"] = pd.to_numeric(net.line.loc[mask, "x_ohm_per_km"], errors="coerce") * 0.7
    ...
    elif variant["variant_id"] == "max_i_2x_x_0p7":
        net.line.loc[mask, "max_i_ka"] = pd.to_numeric(net.line.loc[mask, "max_i_ka"], errors="coerce") * 2.0
        net.line.loc[mask, "x_ohm_per_km"] = pd.to_numeric(net.line.loc[mask, "x_ohm_per_km"], errors="coerce") * 0.7
    elif variant["variant_id"] == "max_i_5x_x_0p5":
        net.line.loc[mask, "max_i_ka"] = pd.to_numeric(net.line.loc[mask, "max_i_ka"], errors="coerce") * 5.0
        net.line.loc[mask, "x_ohm_per_km"] = pd.to_numeric(net.line.loc[mask, "x_ohm_per_km"], errors="coerce") * 0.5
    else:
        raise ValueError(f"Unknown variant: {variant['variant_id']}")
    return {"modified_line_count": int(mask.sum()), "modified_lines": modified["name"].tolist()}
...
def run_case(base_net, variant: dict[str, Any], scale: float) -> dict[str, Any]:
    ...
    row: dict[str, Any] = {
        ...
        "modified_line_count": variant_effects.get("modified_line_count", 0),
        "converged": False,
        "error_type": "",
        "error": "",
        "min_vm_pu": "",
...
def variant_plan() -> list[dict[str, Any]]:
    return [
        ...
        {"variant_id": "max_i_2x", "detail": "Double max_i on ATPL_00075 and ATPL_00147"},
        {"variant_id": "max_i_5x", "detail": "Increase max_i fivefold on ATPL_00075 and ATPL_00147"},
        {"variant_id": "x_0p7", "detail": "Reduce X by 30% on ATPL_00075 and ATPL_00147"},
        {"variant_id": "x_0p5", "detail": "Reduce X by 50% on ATPL_00075 and ATPL_00147"},
        {"variant_id": "max_i_2x_x_0p7", "detail": "Double max_i and reduce X by 30% on both corridors"},
        {"variant_id": "max_i_5x_x_0p5", "detail": "Increase max_i fivefold and reduce X by 50% on both corridors"},
    ]
...
def write_report(summary: dict[str, Any], suggestion_summary: dict[str, Any], results: pd.DataFrame) -> None:
    ...
    if len(converged):
        best = converged.sort_values(["load_scale", "min_vm_pu", "max_line_loading_percent"], ascending=[False, False, True])
    else:
        best = converged
    text = [
        "# 37 S14 Critical Corridor Strengthening",
        "",
        f"Generated: {summary['generated_at']}",
        "",
        f"Scenario: `{SCENARIO_ID}`",
        "",
        "Scope: build on S12 and test whether strengthening ATPL_00075 and ATPL_00147 allows the solver path to extend beyond 10% load. Diagnostic only.",
        "",
        "## Summary",
        "",
        ...
        "",
        markdown_table(best, 100),
        "",
        "## All Attempts",
        "",
        markdown_table(results, 200),
    ]
    (REPORTS / "37_s14_critical_corridor_strengthening.md").write_text("\n".join(text), encoding="utf-8")
...
def main() -> None:
    ...
    results.to_csv(OUT / "s14_corridor_strengthening_attempts.csv", index=False)
    converged = results[results["converged"] == True].copy()
    best = {}
    if len(converged):
    ...
    (OUT / "s14_corridor_strengthening_summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False, default=str), encoding="utf-8")

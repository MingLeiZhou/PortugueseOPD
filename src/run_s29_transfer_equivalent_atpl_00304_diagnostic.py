"""S29 diagnostic scenario: stronger transfer-equivalent treatment for ATPL_00304."""
...
OUT = ROOT / "data" / "processed" / "dcopf_s29_atpl_00304_transfer_equivalent"
...
SCENARIO_ID = "S29_TRANSFER_EQUIVALENT_ATPL_00304_DIAGNOSTIC"
...
def variant_plan() -> list[str]:
    return ["baseline", "parallel_equivalent_2x", "parallel_equivalent_4x", "parallel_equivalent_6x"]
...
def write_report(summary_df: pd.DataFrame, change_df: pd.DataFrame, top_gens_df: pd.DataFrame, top_lines_df: pd.DataFrame) -> None:
    ...
    text = [
        "# 74 S29 Transfer Equivalent ATPL_00304 Diagnostic",
        "",
        f"Generated: {datetime.now(timezone.utc).isoformat()}",
        "",
        f"Scenario: `{SCENARIO_ID}`",
        "",
        "Scope: test whether a stronger transfer-equivalent treatment of ATPL_00304 can materially improve the current best internal DC OPF benchmark candidate.",
        "",
        "## Summary",
        "",
    ...
    (REPORTS / "74_s29_transfer_equivalent_atpl_00304_diagnostic.md").write_text("\n".join(text), encoding="utf-8")
...
def main() -> None:
    ...
    summary_df.to_csv(OUT / "s29_summary.csv", index=False)
    top_gens_df.to_csv(OUT / "s29_top_generators.csv", index=False)
    top_lines_df.to_csv(OUT / "s29_top_lines.csv", index=False)
    change_df.to_csv(OUT / "s29_parameter_changes.csv", index=False)
    (OUT / "s29_summary.json").write_text(json.dumps({"rows": len(summary_df), "status": "DIAGNOSTIC_DONE"}, indent=2), encoding="utf-8")

"""S28 diagnostic scenario: targeted remediation variants for ATPL_00244."""
...
OUT = ROOT / "data" / "processed" / "dcopf_s28_atpl_00244_remediation"
...
SCENARIO_ID = "S28_TARGETED_REMEDIATION_ATPL_00244"
LINE_NAME = "ATPL_00244"
LOAD_SCALE = 0.3
OH_WEIGHT = 0.75
CB_WEIGHT = 0.25
...
def apply_variant(net, variant_id: str) -> dict[str, Any]:
    ...
    if variant_id == "baseline":
        return out
    if variant_id == "weighted_reparam":
    ...
    elif variant_id == "parallel_equivalent":
    ...
    elif variant_id == "aggressive_parallel_equivalent":
        net.line.loc[idx, "max_i_ka"] = pd.to_numeric(net.line.loc[idx, "max_i_ka"], errors="coerce") * 3.0
        net.line.loc[idx, "x_ohm_per_km"] = pd.to_numeric(net.line.loc[idx, "x_ohm_per_km"], errors="coerce") * 0.4
    else:
...
def run_case(proxies: pd.DataFrame, costs: pd.DataFrame, variant_id: str) -> tuple[dict[str, Any], pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    net = load_net()
    diversified = expand_dispatchable_fleet(proxies)
    attach_generators(net, diversified, costs)
    set_load_scale(net, LOAD_SCALE)
    change = apply_variant(net, variant_id)
    ...
    try:
        ...
        return summary, gen_df, line_df, pd.DataFrame([change])
    except Exception as exc:
        summary["error_type"] = type(exc).__name__
        summary["error"] = str(exc)
        return summary, pd.DataFrame(), pd.DataFrame(), pd.DataFrame([change])


def variant_plan() -> list[str]:
    return ["baseline", "weighted_reparam", "parallel_equivalent", "aggressive_parallel_equivalent"]
...
def write_report(summary_df: pd.DataFrame, change_df: pd.DataFrame, top_gens_df: pd.DataFrame, top_lines_df: pd.DataFrame) -> None:
    ...
    text = [
        "# 67 S28 ATPL_00244 Targeted Remediation",
        "",
        f"Generated: {datetime.now(timezone.utc).isoformat()}",
        "",
        f"Scenario: `{SCENARIO_ID}`",
        "",
        "Scope: test whether ATPL_00244 is better treated by weighted mixed reparameterization or by a stronger parallel-equivalent transfer representation under the current best DC OPF baseline.",
        "",
        "## Summary",
        "",
    ...
    (REPORTS / "67_s28_atpl_00244_targeted_remediation.md").write_text("\n".join(text), encoding="utf-8")
...
def main() -> None:
    ...
    summary_df.to_csv(OUT / "s28_summary.csv", index=False)
    top_gens_df.to_csv(OUT / "s28_top_generators.csv", index=False)
    top_lines_df.to_csv(OUT / "s28_top_lines.csv", index=False)
    change_df.to_csv(OUT / "s28_parameter_changes.csv", index=False)
    (OUT / "s28_summary.json").write_text(json.dumps({"rows": len(summary_df), "status": "DIAGNOSTIC_DONE"}, indent=2), encoding="utf-8")

"""Trace ATPL_00304 lineage and OPF bottleneck context."""
...
OUT = PROCESSED / "dcopf_s21_diagnostics"
REPORTS = ROOT / "reports"
LINE_ID = "ATPL_00304"
FROM_FACILITY = "1107P5728100"
TO_FACILITY = "1116S5903800"
...
def main() -> None:
    ...
    opf_lines = read_csv(OUT / "s21_dcopf_line_results_by_scale.csv")
    opf_gens = read_csv(OUT / "s21_dcopf_gen_results_by_scale.csv")
    ...
    opf_context = opf_lines[opf_lines["name"].astype(str) == LINE_ID].copy() if len(opf_lines) else pd.DataFrame()
    top_dispatch = opf_gens.groupby("name", as_index=False)["p_mw_res"].max().sort_values("p_mw_res", ascending=False).head(10) if len(opf_gens) else pd.DataFrame()
        ...
        [
            {
                ...
                "recommended_action": "MANUAL_REVIEW_AND_S22_TARGETED_REMEDIATION",
                "reason": "ATPL_00304 becomes the dominant binding line once the DC OPF is forced to rely on internal proxy generation instead of ext_grid balancing.",
            }
        ]
    )

    lineage.to_csv(OUT / "atpl_00304_lineage.csv", index=False)
    candidate_row.to_csv(OUT / "atpl_00304_candidate_line_row.csv", index=False)
    acpf_row.to_csv(OUT / "atpl_00304_acpf_line_row.csv", index=False)
    interfacility_row.to_csv(OUT / "atpl_00304_interfacility_row.csv", index=False)
    classification_row.to_csv(OUT / "atpl_00304_classification_row.csv", index=False)
    source_segments.to_csv(OUT / "atpl_00304_source_segment_endpoints.csv", index=False)
    endpoint_match_rows.to_csv(OUT / "atpl_00304_endpoint_matches.csv", index=False)
    facility_rows.to_csv(OUT / "atpl_00304_facilities.csv", index=False)
    opf_context.to_csv(OUT / "atpl_00304_opf_context.csv", index=False)
    top_dispatch.to_csv(OUT / "atpl_00304_top_dispatch_context.csv", index=False)
    ...
    (OUT / "atpl_00304_lineage_summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")

    text = [
        "# 56 ATPL_00304 OPF Bottleneck Trace",
        "",
        f"Generated: {summary['generated_at']}",
        "",
        "Scope: trace `ATPL_00304`, the dominant binding line in the S21 internal DC OPF runs, and connect it back to topology and parameter provenance.",
        "",
        "## Lineage Summary",
        "",
        ...
        "## Interpretation",
        "",
        "`ATPL_00304` is another mixed branch still represented by the same 50/50 mixed proxy family. Under internal DC OPF dispatch, it becomes more binding than `ATPL_00075`, which suggests the OPF route is now exposing a new class of supply-path bottlenecks along the Fanhões–Caneças side of the backbone. The next engineering action should be a targeted remediation scenario (`S22`) focused on this corridor rather than more generic OPF tuning.",
    ]
    (REPORTS / "56_atpl_00304_opf_bottleneck_trace.md").write_text("\n".join(text), encoding="utf-8")
